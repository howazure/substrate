...
decl_module! {
		...
		T::AccountId: AsRef<[u8]>,
	{
		...
		/// - None: See `address_from_account_nonce`
		/// - Some(salt): See `address_predictable`
		///
	...
	T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>,
{
	...
	/// Determine the address of a contract in a predictable way.
	///
	/// Formula: `hash(deploying_address ++ le_bytes(account_nonce))`
	/// This is akin to the formular of eth's CRATE opcode.
	pub fn address_from_account_nonce(deploying_address: &T::AccountId) -> T::AccountId
	{
		let buf: Vec<_> = deploying_address.as_ref().iter()
			.chain(&System::<T>::account_nonce(deploying_address).encode())
			.cloned()
			.collect();
	...
	/// Determine the address of a contract in a predictable way.
	///
	/// Formula: `hash(0xff ++ deploying_address ++ le_bytes(salt) ++ code_hash)`
	/// This is akin to the formular of eth's CRATE2 opcode.
	pub fn address_predictable(
		deploying_address: &T::AccountId,
		salt: u64,
		code_hash: &CodeHash<T>,
	) -> T::AccountId
	{
		let buf: Vec<_> = sp_std::iter::once(&0xffu8)
			.chain(deploying_address.as_ref())
			.chain(salt.to_le_bytes().as_ref())
